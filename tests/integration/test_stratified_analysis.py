...
class TestStratifiedAnalysis:
    ...
    def _validate_context_performance(self, results: list, contexts: list):
        ...
        context_metrics = {}
        for result, context in zip(results, contexts, strict=False):
            context_metrics[context["name"]] = result["metrics"]

        # GC-rich contexts should generally perform better
        gc_rich_idx = next(
            i for i, c in enumerate(contexts) if c["name"] == "cpg_islands"
        )
        intergenic_idx = next(
            i for i, c in enumerate(contexts) if c["name"] == "intergenic"
        )
        ...
        assert gc_rich_auc >= intergenic_auc * 0.9, (
            "GC-rich context should perform well"
        )
    ...
    def _validate_depth_performance(self, results: list, depth_configs: list):
        ...
        depth_metrics = {}
        for result, config in zip(results, depth_configs, strict=False):
            depth_metrics[config["name"]] = result["metrics"]

        # Higher depth should generally improve performance
        low_depth_idx = next(
            i for i, c in enumerate(depth_configs) if c["name"] == "low_depth"
        )
        high_depth_idx = next(
            i for i, c in enumerate(depth_configs) if c["name"] == "high_depth"
        )
    ...
    def _validate_af_performance(self, results: list, af_configs: list):
        ...
        af_metrics = {}
        for result, config in zip(results, af_configs, strict=False):
            af_metrics[config["name"]] = result["metrics"]

        # Higher AF should generally perform better
        high_af_idx = next(
            i for i, c in enumerate(af_configs) if c["name"] == "high_af"
        )
        ...
        assert high_auc >= low_auc * 1.2, (
            "Higher AF should perform significantly better"
        )

    def _validate_multi_stratification(self, results: list, multi_strata: list):
        """Validate multi-dimensional stratification results."""
        for result, stratum in zip(results, multi_strata, strict=False):
            ...
            assert metrics["roc_auc"] > 0.7, (
                f"Should maintain good performance in {stratum['name']}"
            )
...
class TestPowerAnalysisStratification:
        ...
        for af in config.simulation.allele_fractions:
            for depth in config.simulation.umi_depths:
                ...
                if len(stratum_calls) > 0:
                    ...
                    sensitivity = (
                        stratum_calls["significant"].mean()
                        if "significant" in stratum_calls.columns
                        else detection_rate
                    )
    ...
    def _validate_power_by_depth(self, result: dict, stratum: dict):
        ...
        if stratum["name"] == "high_depth":
            ...
            low_af_keys = [
                k for k in power_metrics.keys() if "af_0.001" in k or "af_0.0005" in k
            ]
            for key in low_af_keys:
                metrics = power_metrics[key]
                assert metrics["sensitivity"] > 0.7, (
                    f"Should achieve good power in high depth stratum for {key}"
                )
