...
from precise_mrd.determinism_utils import set_global_seed, env_fingerprint
...
class TestDeterministicBehavior:
    ...
    def test_environment_fingerprint_consistency(self):
        ...
        for i in range(1, len(fingerprints)):
            assert fingerprints[0] == fingerprints[i], f"Fingerprint {i} differs from first"
    ...
    def test_artifact_hash_stability(self, temp_dir):
        ...
        # Run pipeline twice
        results1 = self._run_pipeline(config, temp_dir / "run1", seed=config.seed)
        results2 = self._run_pipeline(config, temp_dir / "run2", seed=config.seed)
        ...
        assert manifest1 == manifest2, "Hash manifests should be identical for same inputs"
    ...
    def test_cross_platform_compatibility(self, temp_dir):
        ...
        rng_methods = [
            {"method": "default_rng", "generator": lambda seed: np.random.default_rng(seed)},
        ...
        for rng_info in rng_methods:
            ...
            result = self._run_pipeline_with_rng(config, output_path, rng_info['generator'])
            results.append(result)

        # Results should be consistent (both methods should work)
        for result in results:
            assert result['metrics']['roc_auc'] > 0.5, "Both RNG methods should produce valid results"

    def _run_pipeline(self, config: PipelineConfig, output_path: Path, seed: int) -> dict:
    ...
    def _run_pipeline_with_rng(self, config: PipelineConfig, output_path: Path, rng_generator) -> dict:
        ...
        return {
            ...
            'metrics': metrics,
            'config': config,
            'rng_method': rng_generator.__name__ if hasattr(rng_generator, '__name__') else str(rng_generator)
        }
    ...
    def _assert_all_identical(self, results: list):
        ...
        for i in range(1, len(metrics_list)):
            assert metrics_list[0]['roc_auc'] == metrics_list[i]['roc_auc'], "ROC AUC should be identical"
            assert metrics_list[0]['average_precision'] == metrics_list[i]['average_precision'], "AP should be identical"
    ...
    def _assert_dataframes_identical(self, df1: pd.DataFrame, df2: pd.DataFrame):
        ...
        sort_cols = ['sample_id', 'family_id'] if 'family_id' in df1.columns else ['sample_id']
        ...
        for col in ['sample_id', 'is_variant', 'p_value', 'p_adjusted']:
            if col in df1.columns:
                assert (df1_sorted[col] == df2_sorted[col]).all(), f"Column {col} should be identical"
...
class TestDeterminismRegression:
    ...
    def test_known_good_configuration(self, temp_dir):
        ...
        assert result['metrics']['roc_auc'] > 0.9, "Known good configuration should achieve high ROC AUC"
    ...
    def _assert_results_similar(self, result1: dict, result2: dict, variation: dict):
        ...
        assert 0.9 <= auc_ratio <= 1.1, f"ROC AUC should be similar for {variation['name']} change"
        ...
        if variation['name'] != 'seed':
            ...
            if variation['name'] in ['min_family_size', 'consensus_threshold', 'alpha']:
                ...
                assert different_calls > 0, f"Parameter {variation['name']} should affect results"
