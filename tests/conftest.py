...
from precise_mrd.io import SyntheticReadGenerator, TargetSite, Read
        ...
        for depth in umi_depths:
            for rep in range(20):  # 20 replicates each
                ...
                results.append({
                    ...
                    'replicate_id': rep,
                    'detected': detected,
                    'pvalue': np.random.uniform(0.001, 0.1) if detected else np.random.uniform(0.1, 1.0),
...
def create_test_read(chrom="chr1", pos=1000, ref="A", alt="T", umi="AAGGCCTT", 
                    quality=30, strand="+", read_position=50, family_id=None):
