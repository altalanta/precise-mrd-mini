...
class ErrorModel:
    ...
    def export_error_model(self, filepath: str) -> None:
        ...
        model_data = {
            ...
            'contamination_matrix': {
                f"{k[0]}->{k[1]}": v for k, v in self.contamination_matrix.items()
            }
        }
    ...
    def load_error_model(self, filepath: str) -> None:
        ...
        self.base_error_rate = model_data['base_error_rate']
        self.contamination_rate = model_data['contamination_rate']
        self.context_error_rates = model_data['context_error_rates']
        
        # Reconstruct contamination matrix
        self.contamination_matrix = {}
        for key, value in model_data.get('contamination_matrix', {}).items():
            sample1, sample2 = key.split('->')
            self.contamination_matrix[(sample1, sample2)] = value